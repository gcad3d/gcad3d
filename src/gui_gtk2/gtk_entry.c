...
/// set text in entryField
  ...
  // GdkRGBA col1={0.0, 0.0, 1.0, 1.0};  // blue  Gtk3
  // GdkColor  col1 = {0x0000, 0x0010, 0x0010, 0xc000};
...
// Gtk3:
  // gtk_widget_override_background_color (((Obj_Entry*)go)->entry, 0, &col1);
  // gtk_widget_modify_bg (((Obj_Entry*)go)->entry, GTK_STATE_NORMAL, &col1);
  // gtk_widget_set_style (((Obj_Entry*)go)->entry, UI_stylTab[1]);
...
/// \endcode
  ...
  if(((GdkEvent*)event)->type == GDK_FOCUS_CHANGE) {   // 12
    // GdkEventFocus 
    ikey = 0;
    // imod = ((GdkEventFocus*)event)->in;                always 1
    // printf(" gtk_widget_is_focus=%d\n",gtk_widget_is_focus(go->entry));
