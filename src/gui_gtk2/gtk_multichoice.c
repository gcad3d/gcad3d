...
/// \endcode
  ...
  // GUI_packwn (w_par, eb1, hsiz, vsiz);
  ...
  gtk_container_add (GTK_CONTAINER(fr1), lb1);
  gtk_misc_set_alignment (GTK_MISC (lb1), 0.f, 0.5f); // left
  gtk_widget_show (lb1);
  // change background-color & text w. GUI_OptMenSet 
  // oom->capt = lb1;
...
/// \endcode
  ...
  if(mode == 1) {
    // gtk_widget_override_background_color (parent, 0, &col1);
    gtk_widget_set_style (go->data, UI_stylTab[2]);    // blue
    // gtk_widget_modify_bg (go->data, GTK_STATE_NORMAL, col1);
    // gtk_widget_modify_bg (go->data, GTK_STATE_PRELIGHT, col1);


  } else if(mode == 0) {
    gtk_widget_restore_default_style (go->data);
...
/// \endcode
  ...
  // skip reporting to user if window is coming up
  // if() return 0;
