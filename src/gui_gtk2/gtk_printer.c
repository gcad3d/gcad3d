...
/usr/include/gtk-unix-print-2.0/gtk/ *.h    !!!!!
...
//================================================================
  ...
  PRI_win = gtk_print_unix_dialog_new (NULL, NULL);
/*
  if (gtk_dialog_run(GTK_DIALOG(PRI_win)) == GTK_RESPONSE_OK) {
        prt1 = gtk_print_unix_dialog_get_selected_printer(
GTK_PRINT_UNIX_DIALOG(PRI_win));
...
GTK_PRINT_UNIX_DIALOG(PRI_win));
        ...
        print_job = gtk_print_job_new("Example title", prt1,
settings, page_setup);
  }
*/
  ...
  // pJob = gtk_print_job_new ("Print", printer, settings, pgSetup);
  ...
  if (error)
    {
      ...
      dialog = gtk_message_dialog_new (GTK_WINDOW (PRI_win),
