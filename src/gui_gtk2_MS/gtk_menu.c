...
/// \endcode
  ...
  if(mode == 1)
    gtk_box_pack_start (GTK_BOX (w_par), UI_act_wi, TRUE, TRUE, 0);
  else
    gtk_box_pack_start (GTK_BOX (w_par), UI_act_wi, FALSE, TRUE, 0);
  gtk_widget_show (UI_act_wi);
  goto L_done;
  ...
  if(mode == 'r') {
    gtk_menu_item_set_right_justified (GTK_MENU_ITEM(w1), TRUE);
  }

  gtk_menu_item_set_submenu (GTK_MENU_ITEM (w1), UI_act_wi);

  if(!ltxt) {
    gtk_menu_bar_append (GTK_MENU_BAR (w_par), w1);
  } else {
