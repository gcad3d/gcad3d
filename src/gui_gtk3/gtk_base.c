...
#endif
/*!
\file ../gui_gtk3/gtk_base.c
\brief window, boxes, Basefunctions
\code
...
//#include "../ut/ut_geo.h"                  // Typ_Memspc MemObj
...
static GdkRGBA colR={1.0, 0.1, 0.1, 1.0};  // blue
static GdkRGBA colB={0.1, 0.1, 1.0, 1.0};  // blue

  GdkRGBA *UI_stylTab[3];
...
// Gtk2:
  ...
  //----------------------------------------------------------------
  // modify widget-default-styles
  {
    GdkScreen *screen;
    GdkDisplay *display;
    GtkCssProvider *prov1;
    display = gdk_display_get_default ();
    screen = gdk_display_get_default_screen (display);
    prov1 = gtk_css_provider_new ();
    gtk_style_context_add_provider_for_screen (screen,
                                             GTK_STYLE_PROVIDER (prov1),
                                             GTK_STYLE_PROVIDER_PRIORITY_USER);
    gtk_css_provider_load_from_data (GTK_CSS_PROVIDER (prov1),
      "GtkEntry {border-width: 2px}", // borderWidth for entry
      -1, NULL);
...
/// \endcode
  ...
  if(hsiz == 0) {
    if(vsiz == 0) {
      gtk_container_add (GTK_CONTAINER (parent), wi);
      goto L_done;
...
*/
  ...
  if(*iex == 0) {
    gtk_widget_set_hexpand (cObj, TRUE);
  } else {
    gtk_widget_set_hexpand (cObj, FALSE);
  }

  if(*iey == 0) {
    gtk_widget_set_vexpand (cObj, TRUE);
  } else {
    gtk_widget_set_vexpand (cObj, FALSE);
  }
...
/// \endcode
  ...
  if(typ == 0) {
    ...
    GUI_siz_set (box0, spc, 0);  // set size
  } else {
    box0 = gtk_box_new (GTK_ORIENTATION_VERTICAL, 0);
    GUI_siz_set (box0, 0, spc);  // set size
  }

  gtk_container_add (GTK_CONTAINER (w_par), box0);
...
/// \endcode
  ...
  if(typ == 0) {
    ...
    w1 = gtk_separator_new (GTK_ORIENTATION_VERTICAL);
  }
...
/// \endcode
  ...
  else if(mode == 2) return gtk_widget_is_drawable (wo);
