...
#endif
/*!
\file ../gui_gtk3/gtk_button.c
\brief button, checkbox
\code
...
// see GUI_label_styl
  ...
  // change button-text
  gtk_widget_override_color (go->widget, 0, UI_stylTab[iCol]);

  // gtk_widget_override_background_color (go->widget, 0, &col1);  // geht ned ...
...
// calls userfunc with pTab;
//    pTab[0]=TYP_EventPress|TYP_EventRelease;
//    pTab[1]=userData ..


  int        i1, pTyp, iTyp=TYP_EventPress;
  void       *pTab[2];
  ...
  if(pTyp == TYP_GUI_CheckBox) {
    i1 = gtk_toggle_button_get_active (parent);
    if(!i1) {
