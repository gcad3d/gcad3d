...
/// \endcode
  ...
  UI_act_wi = box0;
// GUI_set_enable (o_par, 0);
  GUI_list1__ (NULL, fNam, mode, titLst, funcnam, opts);


  L_dlg_lst_cr:
  if(o_par) {
    gtk_window_set_modal (GTK_WINDOW (UI_list1_win), FALSE);
...
//================================================================
  ...
  // inhibit calling GUI_list1_cbSel for each dleted line ..
  GUI_list1_msbt = 0;
...
/// \endcode
  ...
  if (gtk_tree_selection_get_selected (selection, &model, &iter)) {
    ...
    // unselect first (automatic) selected line
    if(!GUI_list1_msbt) {
      gtk_tree_selection_unselect_iter (selection, &iter); //2013-04-10
      return FALSE;
    }
...
/// \endcode
  ...
  // sig changed: from mouse-select, key-blank, key-PageDwn !
  // g_signal_connect (G_OBJECT (select), "clicked",   clicked invalid !
  g_signal_connect (G_OBJECT (select), "changed",
  ...
  // GUI_packwn (box0, UI_List_p3, 0, 0);
