...
UTO_objDat_dbo          get dataStruct from DB-Obj; keep Refs.
...
UTO_par__pt_obj        get parameter from point on curve
...
// UTO_par__pt_dbo           get parameter from point (obj from typ/ind)
// Input:
//   mode     0  get knotvalues for splines
//            1  get parametervalues from 0 -1 also for splines
// ACHTUNG: lpar muss im Bereich 0-1 sein; wird fuer Curves umgerechnet !!
...
// \endcode
  ...
  UTO_par__pt_obj (lpar, mode, pti, ox1.form, ox1.data);
...
// LimitTyp: see UTO_ck_curvLimTyp
  ...
  // DEB_dump_obj__ (typ, oo, "UTO_set_ptlim typ=%d",typ);
  ...
  switch (typ) {
    ...
    case Typ_CVPOL:
      ...
      d1 = FLT_32_MAX;
      if(v1) d1 = UT3D_par_par1plg (*v1, (CurvPoly*)oo);
      else if(p1) UT3D_parplg_plgpt (&d1, p1, (CurvPoly*)oo);
    ...
    // TESTBLOCK
    // DEB_dump_obj__ (typ, oo, "ex-UTO_set_ptlim typ=%d",typ);
    // TESTBLOCK
...
/// \endcode
  ...
  switch(typ) {
    ...
    // (ex UTO_objDat_dbo ein ObjGX liefern)
    case Typ_CV:
    case Typ_SUR:
    case Typ_SOL:
    case Typ_GTXT:
...
//================================================================
// UTO_objDat_dbo                          get standardCurve from DB-curve S
...
//
// see also UTO_objDat_dbo UTO_objDat_ox DB_GetObjDat
  ...
  if(ox1p->form == Typ_CVTRM) {
    ...
    // get trimmed-curve vp1; set to curve[iSeg1]  (UT3D_obj_ccv_segnr)
    if((iSeg1 < 1)          ||
...
/// \code
///
/// UTO_objDat_dbo              get DB-data-struct from typ, DB-index
...
///   oDat             Pointer auf die korrekte struct - NICHT auf ObjGX !
...
/// UTO_objDat_dbo     get dataStruct from DB-Obj Keep Refs.   Give pointer.
...
*/
        // printf("ex UTO_objDat_dbo oTyp=%d dbInd=%ld oNr=%d\n",*oTyp,dbInd,*oNr);
      break;
...
*/
    ...
    default:
      TX_Error("UTO_objDat_dbo: E001 %d",*oTyp);
      return -1; // Par_err;
  }
...
//==============================================================================
/// \code
/// UTO_par__pt_obj        get parameter 0-1 from point on curve
/// Input:
///   mode      0  get knotvalues for splines
///             1  get parametervalues from 0 -1 for splines, polygon, trimmed-curve
/// Output:
///   po           parameter 0-1 of point pti on curve oTyp,oDat
/// \endcode
...
// UT3D_parCv_bsplpt UT3D_par1_parcbsp UT3D_par1_parBSp UT3D_parbsp_par1
  ...
  if(oTyp == Typ_Val) {
    ...
    *po = UT3D_par1_ci_pt ((Circ*)oDat, pti);
    ...
    irc = UT3D_parCv_bsplpt (&d2, &d1, (CurvBSpl*)oDat, pti);
    if(irc != 0) return -1;
    // get po = parameter 0-1 from knotval d2
    if(mode) *po = UT3D_par1_parbsp (&d2, oDat);
    else     *po = d2;
    ...
    if(mode) *po = UT3D_par1_par_rbsp (&d1, (CurvRBSpl*)oDat);
    else     *po = d1;
...
/// see UTO_objDat_dbo    get dataStruct from DB-Obj Keep Refs.   Give pointer.
/// see DB_GetObjGX DB_GetObjDat UTO_objDat_dbS DB_GetObjDat
/// \endcode
...
static ObjGX  *odb;
  ...
  *oNr = ox1->siz;
  ...
  //------------------------------------------
    ...
    if(ox1->form == Typ_Index) {
      DB_GetRef ((Plane*)&objOut, LONG_PTR(ox1->data));
...
// RetCod = form !
// 
// see UTO_objDat_dbo UTO_objDat_ox UTO_obj_get
...
// see UTO_objDat_dbo UTO_objDat_ox
...
#define TABSIZ 20
  ...
  o0Typ = oi->typ;
  irc = UTO_objDat_dbo (&obj0, &i1, &o0Typ, oi->dbInd);
  if(irc < 0) {
...
*/
  ...
  if((iClo == 1) && (o2Typ == Typ_Error)) {
    ...
    // irc = UTO_2pt_limstru (&pt1, &pt2, &v1, &v2, o0Typ, obj0);
    // get startpoint
    irc = UT3D_ptvcpar1_std_obj (&pt1, NULL, &v1, Ptyp_start, o0Typ, obj0);
    if(irc < 0) return irc;
  ...
  if((iClo == 1) && (o2Typ == Typ_Error)) {
    ...
    // get endpoint
    irc = UT3D_ptvcpar1_std_obj (&pt2, NULL, &v2, Ptyp_end, o0Typ, obj0);
    if(irc < 0) return irc;
    // add endpoint as last point into pa/va.
    if((TABSIZ - pNr) < 1) goto TabSizErr; 
    pa[pNr] = pt2;
  ...
  if(p2Nr == 0) {
    irc = UT3D_ptvcpar1_std_obj (&pt1, NULL, &v1, Ptyp_end, o0Typ, obj0);
    if(irc < 0) return irc;
      // DEB_dump_obj__ (Typ_PT, &pt1, " _limstru-pt1 v1=%lf",v1);
      // DEB_dump_obj__ (Typ_PT, &pt2, " pt2 v2=%lf",v2);
      // GR_Disp_pTab (1, &pt1, SYM_TRI_S, 2);
  ...
  *otyp = cvtrm->typ;    // 2014-07-20
  ...
  if(cvtrm->typ != Typ_LN) {
    ...
    if(!cvi) {   // 2016-06-26
      ...
      irc = UTO_objDat_dbo (&cvi, &oNr, otyp, cvtrm->dbi);
      if(irc < 0) return -1;
  ...
  switch (*otyp) {
    ...
    case Typ_LN:        // Line
      ...
      if(cvtrm->ip0) {
        // irc = UTO_objDat_dbo ((void**)&pp1, &oNr, &pTyp, cvtrm->ip0);
        // ((Line*)cvo)->p1 = *pp1;
      ...
      if(cvtrm->ip1) {
        // irc = UTO_objDat_dbo ((void**)&pp1, &oNr, &pTyp, cvtrm->ip1);
        // ((Line*)cvo)->p2 = *pp1;
...
*/
    ...
    // DEB_dump_obj__ (*otyp, cvo, "ex-UTO_cv_cvtrm");
    // printf("____________________________________ \n");
