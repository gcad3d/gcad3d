...
*/
  ...
  // get srcLineNr
  DL_Get_lNr_dli (&actlNr, dli);    printf(" actlNr=%ld\n",actlNr);
  ...
  actAtt = DL_get_iatt (actDli);


    printf("############# L_ini_obj: typ=%d dbi=%ld dli=%ld\n",typ,dbi,dli);
  ...
  sele_reset ();
  // init modify - only sysmbols selectable
  // sele_set__ (Typ_SymB);
  ...
  EDMPT_get_offObj ();        // get offsetObject (Plane or Vector)
...
// modifys the active point (actPta[actInd])
  ...
  if(EDMPT_offTyp == Typ_Error) {   // not initialized ..
    // MSG_pri_0 ("PEDe2");
    return 0;
  }
  ...
  if(actInd < 0) {EDMPT_msg_mode (); goto L_exit;}   // no point sel.
...
// // mode=10   remove all pointSymbols (yellow rect's) in DL
  ...
  //----------------------------------------------------------------
  L_mode6:           // mode=6    redraw curve with attribut ii
...
// create Toolbar
  ...
  EDMPT_w_off = GUI_entry__(&EDMPT_f_pln, "  ", "RZ", 
                          EDMPT_off_CB, NULL, "8");
...
//=====================================================================
  ...
  //-------------------------------------------------
    ...
    // if(EDMPT_offLen == 0.) {
      // GUI_set_enable (&EDMPT_f_mod, TRUE);   // ctivate delete insert modify
    // } else {
    // EDMPT_mode = 3;
  ...
  //-------------------------------------------------
    ...
    EDMPT_msg_mode ();
    sele_reset ();
    sele_set__ (Typ_SymB);            // enable selection only of point-symbols
    sele_setNoConstrPln ();
    // UI_CursorNo (0);
  ...
  //-------------------------------------------------
    ...
    sele_set__ (Typ_SymB);      // enable selection of next point
    EDMPT_msg_mode ();
...
//  - redraw all starting with old codeline
  ...
  DL_set_iatt (actDli, actAtt);        // restore original atribute
...
// EDMPT_restart_lnk  restart with new obj
  ...
  DL_SetInd (actDli);
  APT_Draw__ (actAtt, basTyp, actDbi);
